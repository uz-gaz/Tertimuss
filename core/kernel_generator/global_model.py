...
def generate_global_model(tasks_model: TasksModel, processor_model: ProcessorModel, thermal_model: ThermalModel,
    ...
    b = np.concatenate((tasks_model.c_tau_alloc, processor_model.c_proc_alloc, thermal_model.ct_exec), axis=0)

    bp = np.concatenate(
        (np.zeros((len(tasks_model.a_tau), 1)), np.zeros((len(processor_model.c_proc_alloc), 1)),
         thermal_model.b_ta.reshape((len(thermal_model.b_ta), 1))),
        axis=0)

    s = np.concatenate((processor_model.s_busy, processor_model.s_exec), axis=0)
    ...
    mo = np.concatenate((tasks_model.m_tau_o, processor_model.m_proc_o, mo), axis=0)
