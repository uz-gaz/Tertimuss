...
def solve_global_model(global_model: GlobalModel, mo: scipy.ndarray, w_alloc: scipy.ndarray, ma: float,
    ...
    res = scipy.integrate.solve_ivp(lambda t, m: global_model.a.dot(m.transpose()) + aux, list(range(step)), mo,
                                    dense_output=False)
...
def solve_global_model_new(global_model: GlobalModel, mo: scipy.ndarray, w_alloc: scipy.ndarray, ma: float,
    ...
    :return:
    """
    # TODO: Revisar y hacer tests, actualmente funciona mal
    a = global_model.a
    b = global_model.b
    d = global_model.bp
    c = global_model.s
    ...
    tout = scipy.asarray(list(range(step)))  # scipy.arange(t0, t1, step)
