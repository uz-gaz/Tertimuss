...
def solve_global_model(global_model: GlobalModel, mo: np.ndarray, w_alloc: np.ndarray, ma: float,
    ...
    res = scipy.integrate.solve_ivp(
        lambda t, m: global_function(m, global_model.a, global_model.b, global_model.bp.reshape(len(global_model.bp)),
                                     w_alloc, ma), time_sol, mo
...
def global_function(m: np.ndarray, a: np.ndarray, b: np.ndarray, bp: np.ndarray, w_alloc: np.ndarray, ma: float):
    return a.dot(m.transpose()) + b.dot(w_alloc) + bp * ma
