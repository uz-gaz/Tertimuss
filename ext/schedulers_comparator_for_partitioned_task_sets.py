...
def generate_partitioned_task_set(full_experiment_name: str, partition_description: List[Tuple[int, int]]):
    """
    Generate a random task set from the requirements and return the experiment full name
    :param full_experiment_name: name of the experiment
    :param partition_description: List of (number of cpus, number of tasks)
    :return:
    """
    ...
    x = u.generate(
        {
            ...
            "partition_description": [(number_of_tasks_partition, number_of_cpus_partition) for
                                      (number_of_cpus_partition, number_of_tasks_partition) in partition_description],
...
if __name__ == '__main__':
    ...
    # Generate task sets
    for number_of_cpus_list, number_of_tasks_list in task_number_processors:
        number_of_cpus = sum(number_of_cpus_list)
        number_of_tasks = sum(number_of_tasks_list)
        ...
        for i in range(first_experiment_numeration, first_experiment_numeration + total_number_of_experiments):
            ...
            generate_partitioned_task_set(experiment_name, list(zip(number_of_cpus_list, number_of_tasks_list)))
