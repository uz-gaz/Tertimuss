...
class TasksModel(object):
    ...
    def __init__(self, tasks_specification: TasksSpecification, cpu_specification: CpuSpecification,
        ...
        pre = numpy.sparse.lil_matrix((p, t_w), dtype=simulation_specification.type_precision)
        post = numpy.sparse.lil_matrix((p, t_w), dtype=simulation_specification.type_precision)
        pi = numpy.sparse.lil_matrix((p, t_w), dtype=simulation_specification.type_precision)
        ...
        pre[:n_periodic, :] = numpy.sparse.identity(n_periodic, dtype=simulation_specification.type_precision,
                                                    format="lil")  # Transition from p^w_i to t^w_i
        post[:n_periodic, :] = numpy.sparse.identity(n_periodic, dtype=simulation_specification.type_precision,
                                                     format="lil")  # Transition from t^w_i to p^w_i
        post[n_periodic: 2 * n_periodic, :] = numpy.sparse.identity(n_periodic,
        ...
        lambda_vector[:] = numpy.asarray([task.t for task in tasks_specification.periodic_tasks],
                                         dtype=simulation_specification.type_precision)
        pi[:n_periodic, :] = numpy.sparse.identity(n_periodic, dtype=simulation_specification.type_precision,
                                                   format="lil")  # Transition from p^w_i to t^w_i
        ...
        pre_alloc = numpy.sparse.lil_matrix((p, t_alloc), dtype=simulation_specification.type_precision)
        post_alloc = numpy.sparse.lil_matrix((p, t_alloc), dtype=simulation_specification.type_precision)
        pi_alloc = numpy.sparse.lil_matrix((p, t_alloc), dtype=simulation_specification.type_precision)
        # Lambda vector alloc will be defined in processor model

        # Construction of Pre an Post matrix for Transitions alloc
        pre_alloc[n_periodic:, :] = numpy.sparse.hstack([numpy.sparse.identity(n_periodic + n_aperiodic,
                                                                               dtype=simulation_specification.type_precision,
        ...
        self.pre_tau: numpy.sparse.csr_matrix = pre.tocsr()
        self.post_tau: numpy.sparse.csr_matrix = post.tocsr()
        self.pi_tau: numpy.sparse.csr_matrix = pi.tocsr()
        self.lambda_vector_tau = lambda_vector
        self.mo_tau = mo

        # Definition of the union between the task model and the processor model
        self.pre_alloc_tau: numpy.sparse.csr_matrix = pre_alloc.tocsr()
        self.post_alloc_tau: numpy.sparse.csr_matrix = post_alloc.tocsr()
        self.pi_alloc_tau: numpy.sparse.csr_matrix = pi_alloc.tocsr()
