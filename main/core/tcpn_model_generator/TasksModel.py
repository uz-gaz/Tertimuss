...
class TasksModel(object):
    ...
    def __init__(self, tasks_specification: TasksSpecification, cpu_specification: CpuSpecification,
        ...
        lambda_vector = scipy.zeros(t_w, dtype=simulation_specification.type_precision)
        mo = scipy.zeros((p, 1), dtype=simulation_specification.type_precision)
        ...
        lambda_vector[:] = scipy.asarray([task.t for task in tasks_specification.periodic_tasks],
                                         dtype=simulation_specification.type_precision)
