...
class RUNAScheduler(AbstractScheduler):
    def __init__(self, polling_server: PeriodicTask = None):
        self.__parents_of_tree = None
        self.__dt = None
        self.__operating_frequency = None
        self.__last_tasks_assignation = None
        self.__m = None
        self.__polling_server = polling_server

        # Identifier for the pooling server
        self.__polling_server_id = -3

        # Aperiodic tasks list to be executed
        self.__aperiodic_tasks_executables: List[SystemTask] = []
    ...
    def offline_stage(self, global_specification: GlobalSpecification, periodic_tasks: List[SystemPeriodicTask],
        ...
        task_set = task_set if self.__polling_server is None else task_set + [
            RUNTask(self.__polling_server_id, self.__polling_server.c,
                    int(self.__polling_server.d * selected_frequency))]
    ...
    def schedule_policy(self, time: float, executable_tasks: List[SystemTask], active_tasks: List[int],
        ...
        # Clear aperiodic task list
        executable_tasks_ids = [i.id for i in executable_tasks]
        self.__aperiodic_tasks_executables = [i for i in self.__aperiodic_tasks_executables if
                                              i.id in executable_tasks_ids]

        # Select next aperiodic task to execute
        # As aperiodic tasks are not parallelize, EDF is the best strategy
        next_aperiodic_task_to_execute = min(self.__aperiodic_tasks_executables, key=lambda x: x.next_deadline) if len(
            self.__aperiodic_tasks_executables) != 0 else None
        next_aperiodic_task_to_execute_id = next_aperiodic_task_to_execute.id \
            if next_aperiodic_task_to_execute is not None else -1

        # Set polling server task to empty CPU
        iteration_result = [i if i != self.__polling_server_id else next_aperiodic_task_to_execute_id
    ...
    def aperiodic_arrive(self, time: float, aperiodic_tasks_arrived: List[SystemTask],
        ...
        self.__aperiodic_tasks_executables = self.__aperiodic_tasks_executables + aperiodic_tasks_arrived
