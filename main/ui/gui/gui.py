...
from main.core.tcpn_model_generator.global_model import generate_global_model
...
class SimulationSpecificationControl(ttk.Frame):
    ...
    def __init__(self, parent, enable_thermal: bool):
        """
        :param parent: The parent window
        """
        super().__init__(parent)

        # Thermal or not thermal simulation
    ...
    def get_specification(self) -> SimulationSpecification:
        ...
        if len(invalid_fields) > 0:
            ...
            step = float(self.step_entry.get()) if self.enable_thermal else None
...
class TaskSpecificationControl(ttk.Frame):
    ...
    def __init__(self, parent, internal_error_handler: Callable[[List[str]], None], enable_thermal: bool):
        """
        :param parent: The parent window
        """
        super().__init__(parent)
        ...
        self.manual_task_generation_label.grid(column=0, row=0, columnspan=3 if self.enable_thermal else 2, pady=10)

        # Task list
        self.tasks_list = ttk.Treeview(self, columns=("t", "e") if self.enable_thermal else ("t"))
    ...
    def add_task_callback(self, internal_error_handler: Callable[[List[str]], None]):
        ...
        if len(invalid_fields) == 0:
            self.tasks_list.insert("", tk.END, text=self.c_entry.get(),
                                   values=(self.t_entry.get(), self.e_entry.get()) if self.enable_thermal else (
                                       self.t_entry.get()))
    ...
    def generate_automatic_tasks(self, internal_error_handler: Callable[[List[str]], None]):
        ...
        if len(invalid_fields) > 0:
            ...
            # Add new tasks
            for i in tasks:
                self.tasks_list.insert("", tk.END, text=i.c, values=(i.t, i.e) if self.enable_thermal else (i.t))
...
class CpuSpecificationControl(ttk.Frame):
    ...
    def __init__(self, parent, internal_error_handler: Callable[[List[str]], None], enable_thermal: bool):
        ...
        if self.enable_thermal:
            ...
            self.x_board_entry = ttk.Entry(self.frame_interior_2, validatecommand=float_positive_validator,
                                           validate='none')
            ...
            self.y_board_entry = ttk.Entry(self.frame_interior_2, validatecommand=float_positive_validator,
                                           validate='none')
            ...
            self.z_board_entry = ttk.Entry(self.frame_interior_2, validatecommand=float_positive_validator,
                                           validate='none')
            ...
            self.p_board_entry = ttk.Entry(self.frame_interior_2, validatecommand=float_positive_validator,
                                           validate='none')
            ...
            self.k_board_entry = ttk.Entry(self.frame_interior_2, validatecommand=float_positive_validator,
                                           validate='none')
            ...
            self.x_cpu_entry = ttk.Entry(self.frame_interior_3, validatecommand=float_positive_validator,
                                         validate='none')
            ...
            self.y_cpu_entry = ttk.Entry(self.frame_interior_3, validatecommand=float_positive_validator,
                                         validate='none')
            ...
            self.z_cpu_entry = ttk.Entry(self.frame_interior_3, validatecommand=float_positive_validator,
                                         validate='none')
            ...
            self.p_cpu_entry = ttk.Entry(self.frame_interior_3, validatecommand=float_positive_validator,
                                         validate='none')
            ...
            self.c_p_cpu_entry = ttk.Entry(self.frame_interior_3, validatecommand=float_positive_validator,
                                           validate='none')
            ...
            self.k_cpu_entry = ttk.Entry(self.frame_interior_3, validatecommand=float_positive_validator,
                                         validate='none')
    ...
    def get_specification(self) -> CpuSpecification:
        ...
        if self.enable_thermal:
            ...
            if len(invalid_fields) > 0:
                ...
                origins = [
                    Origin(float(self.origins_list.item(i).get("text")),
                           float(self.origins_list.item(i).get("values")[0]))
            ...
            if len(invalid_fields) > 0:
                ...
                m = int(self.m_entry.get())
                f = float(self.f_entry.get())
                return CpuSpecification(None, None, m, f, None)
...
class OutputControl(ttk.Frame):
    ...
    def get_specification(self) -> List[OutputControlOptions]:
        ...
        if self.enable_thermal:
            ...
            if self.execution_and_task_allocation_combobox.get() == "":
                invalid_fields += ["Output"]
            elif self.execution_and_task_allocation_combobox.get() == "Not display or save":
                invalid_fields += ["Output, at least one must be displayed or saved"]
            # Return
            if len(invalid_fields) > 0:
                raise InputValidationError(invalid_fields)
                ...
                return [combobox_values.get(self.execution_and_task_allocation_combobox.get()),
                        self.OutputControlOptions.NOTHING]
...
class SpecificationCategoriesControl(ttk.Frame):
    ...
    def __init__(self, parent, internal_message_handler: Callable[[str], None], enable_thermal: bool):
        super().__init__(parent)
        ...
        self.simulation_content = SimulationSpecificationControl(self.notebook, self.enable_thermal)
        ...
        self.scheduler_content = SchedulerSpecificationControl(self.notebook, self.enable_thermal)
        ...
        self.output_content = OutputControl(self.notebook, self.enable_thermal)
...
class GraphicalUserInterface(ttk.Frame):
    def __init__(self, parent, enable_thermal: bool):
        ...
        # Tabs panel
        self.tabs_panel = SpecificationCategoriesControl(self, self.message_show_handler, enable_thermal)
    ...
    def run_simulation(self):
        try:
            ...
            environment_specification = self.tabs_panel.environment_content.get_specification() if self.enable_thermal else None
            ...
            thermal_model: Optional[ThermalModel] = generate_thermal_model(tasks_specification, cpu_specification,
                                                                           ...
                                                                           simulation_specification) if self.enable_thermal else None
            ...
            try:
                ...
                p = Process(target=self.plot_output,
                            args=(global_specification, self.enable_thermal, output_specification,
