...
from tertimuss_simulation_lib.system_definition import TaskSet, HomogeneousCpuSpecification
...
class ProcessorModel(object):
    ...
    def __init__(self, cpu_specification: Union[HomogeneousCpuSpecification],
        ...
        pre_exec = scipy.sparse.lil_matrix((p, t_exec), dtype=simulation_precision)
        post_exec = scipy.sparse.lil_matrix((p, t_exec), dtype=simulation_precision)
        pi_exec = scipy.sparse.lil_matrix((p, t_exec), dtype=simulation_precision)
        ...
        for k in range(m):
            ...
            pi_exec[i:i + n, k * n: k * n + n] = scipy.sparse.identity(n, dtype=simulation_precision,
                                                                       format="lil")
